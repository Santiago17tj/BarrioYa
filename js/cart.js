/* ==========================================================
   ...
   Persistent shopping cart with localStorage + off-canvas panel
   ========================================================== */
...
class CartManager {
  ...
  getTotal() {
    return this.items.length > 0 ? this.getSubtotal() + CartManager.DELIVERY_FEE : 0;
  }
  ...
  generateOrderJSON() {
    ...
    return {
      ...
      subtotal,
      delivery_fee: CartManager.DELIVERY_FEE,
      total: subtotal + CartManager.DELIVERY_FEE,
...
(function initCartUI() {
  // Don't render on admin or tracking pages
  ...
  const cartHTML = `
    ...
    <div class="cart-panel" id="cartPanel">
      ...
      <div class="cart-panel-footer" id="cartPanelFooter">
        <!-- Totals + CTAs rendered dynamically -->
      </div>
    </div>
  `;
  ...
  // ── Empty State SVG ──
  const EMPTY_SVG = `
    <svg xmlns="http://www.w3.org/2000/svg" width="120" height="120" viewBox="0 0 24 24" fill="none" stroke="var(--color-primary-lighter, #C8E6C9)" stroke-width="1" stroke-linecap="round" stroke-linejoin="round" style="opacity:0.6">
      <circle cx="9" cy="21" r="1"/><circle cx="20" cy="21" r="1"/>
      <path d="M1 1h4l2.68 13.39a2 2 0 0 0 2 1.61h9.72a2 2 0 0 0 2-1.61L23 6H6"/>
    </svg>`;
  ...
  function renderCart() {
    if (cart.isEmpty()) {
      body.innerHTML = `
        <div class="cart-empty">
          ...
          <p>Explora los negocios del barrio y agrega productos increíbles 🏪</p>
          <a href="servicios.html" class="btn btn-primary" style="margin-top:1rem;font-size:0.85rem;padding:0.65rem 1.5rem;" onclick="document.getElementById('cartPanel').classList.remove('open'); document.getElementById('cartOverlay').classList.remove('open'); document.body.style.overflow='';">
            Ver servicios
          </a>
        </div>`;
      footer.innerHTML = '';
      ...
      ${cart.items.map(item => `
        <div class="cart-item" data-name="${item.name}">
          ...
          <div class="cart-item-controls">
            <button class="qty-btn" onclick="cartUI.updateQty('${item.name}', -1)" aria-label="Reducir cantidad">−</button>
            <span class="qty-value">${item.qty}</span>
            <button class="qty-btn" onclick="cartUI.updateQty('${item.name}', 1)" aria-label="Aumentar cantidad">+</button>
          </div>
          <div class="cart-item-total">${fmt(item.price * item.qty)}</div>
          <button class="cart-item-remove" onclick="cartUI.removeItem('${item.name}')" aria-label="Eliminar producto">
    ...
    footer.innerHTML = `
      <div class="cart-summary">
        ...
        <div class="cart-summary-row">
          <span>🛵 Envío</span><span>${fmt(CartManager.DELIVERY_FEE)}</span>
        </div>
      ...
      <div class="cart-actions">
        ...
        <a href="bot-demo.html?fromCart=1" class="cart-whatsapp-btn" id="cartWhatsAppBtn">
  ...
  window.cartUI = {
    ...
    addProduct(product, businessName) {
      cart.addToCart(product, businessName);
      updateBadge();
